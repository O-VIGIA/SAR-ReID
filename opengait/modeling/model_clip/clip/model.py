...
def resize_pos_embed(posemb, posemb_new, height, width):
    ...
    print(f"Resizing CLIP position embedding: {tuple(posemb.shape)} -> {tuple(posemb_new.shape)}")

    posemb_token, posemb_grid = posemb[:1], posemb[1:]

    gs_old = int(math.sqrt(len(posemb_grid)))
    print(f"Target CLIP patch grid: height={height}, width={width}")
    posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
    posemb_grid = F.interpolate(
        posemb_grid, size=(height, width), mode='bilinear', align_corners=False
    )
    posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, height * width, -1)
    posemb = torch.cat([posemb_token, posemb_grid[0]], dim=0)
    return posemb
